...
export class ConcurrencyPool<T> {
  ...
  private async executeTask(task: Task<T>, attempt: number = 0): Promise<void> {
    ...
    try {
      ...
      if (attempt === 0 || DEBUG) {
        console.error(
          `[POOL] Erro na tarefa ${taskId.substring(0, 50)}... (tentativa ${attempt + 1}/${this.maxRetries + 1}): ${errorMessage}`
        )
      ...
      console.error(
        `[POOL] Falha final após ${attempt + 1} tentativas: ${taskId.substring(0, 50)}... - ${errorMessage}`
      )
