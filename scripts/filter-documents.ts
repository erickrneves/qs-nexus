...
import { markFileRejected, getFileByPath } from '../lib/services/file-tracker.js'
 ...
 */
async function filterDocumentTask(
  ...
  if (file.wordsCount < MIN_WORDS) {
    await markFileRejected(
      file.filePath,
      `Muito pequeno: ${file.wordsCount} palavras (mínimo: ${MIN_WORDS})`
    )
    return {
      filePath: file.filePath,
      accepted: false,
  ...
  if (file.wordsCount > MAX_WORDS) {
    await markFileRejected(
      file.filePath,
      `Muito grande: ${file.wordsCount} palavras (máximo: ${MAX_WORDS})`
    )
    return {
      filePath: file.filePath,
      accepted: false,
...
async function main() {
  ...
  const rejected = results.filter(
    r => r.success && r.result && !r.result.accepted && !r.result.skipped
  ).length
