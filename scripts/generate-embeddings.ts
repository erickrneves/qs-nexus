 ...
 */
async function generateEmbeddingsTask(
  template: InferSelectModel<typeof templates>
): Promise<EmbedResult> {
  ...
  try {
    ...
    const isTokenLimitError =
      errorMessage.includes('maximum context length') || errorMessage.includes('8192 tokens')
...
async function main() {
  ...
  const tokenLimitErrors = results.filter(
    r => !r.success && r.result?.errorType === 'token_limit'
  ).length
  ...
  if (problematicTemplates.length > 0) {
    ...
    problematicTemplates.slice(0, 10).forEach(id => {
      ...
      const errorType =
        result?.result?.errorType === 'token_limit' ? 'limite de tokens' : 'outro erro'
